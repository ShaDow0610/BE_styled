...
export async function DELETE(request: NextRequest, { params }: Params) {
  try {
    ...
    const inUse = await Product.exists({ fournisseur_id: id });
    if (inUse) {
      return NextResponse.json(
        { success: false, error: 'Ce fournisseur est utilisé par au moins un produit — réaffecte-les avant de le supprimer.' },
        { status: 400 }
      );
    }
